...
export interface Room {
	...
	height: number
	floorColor: Color
	/** Сетка потолочных ламп: [колонок по X, рядов по Z]. */
	lamps: [number, number]
}

export interface DoorOpening {
	/** Координата центра вдоль стены. */
	at: number
	width: number
	height: number
	kind: "door" | "double" | "arch" | "glass"
}
...
export interface Wall {
	/** Направление, вдоль которого тянется стена. */
	axis: "x" | "z"
	/** Координата стены по второй оси (центр толщины). */
	at: number
	from: number
	to: number
	thickness: number
	...
	/** Панели внизу стены, как в настоящей школе. */
	wainscot?: boolean
}
...
function classroom(id: string, subject: string, x0: number): Room {
	return {
		id,
...
function classroomWindows(x0: number): WindowOpening[] {
...
export interface SpawnPoint {
	...
	x: number
	z: number
	/** 0 — взгляд в −Z (вглубь школы), +π/2 — в −X. */
	yaw: number
}
...
export function findRoomAt(x: number, z: number): Room | null {
...
export function findSpawn(id: string | null | undefined): SpawnPoint {
	if (!id) return DEFAULT_SPAWN
