...
import { FACE, hashNoise, shade, type Color, type MeshBuilder } from "../core/mesh.js"
...
export function ceilingLamp(ctx: Ctx, x: number, z: number, y: number, long = 1.2): void {
...
export function desk(ctx: Ctx, x: number, z: number, angle: number): void {
	const top = 0.75
...
export function chair(ctx: Ctx, x: number, z: number, angle: number, color = PALETTE.plasticBlue): void {
...
/** Учительский стол. */
export function teacherDesk(ctx: Ctx, x: number, z: number, angle: number): void {
	...
	if (axis === "x") {
		...
		for (let i = 0; i < count; i++) {
			...
			decor(
				...
				handleX - 0.025,
				1.0,
				Math.min(doorZ, doorZ + facing * 0.03),
				handleX + 0.025,
				1.16,
				Math.max(doorZ, doorZ + facing * 0.03),
				PALETTE.metal,
			)
		}
	} else {
		...
		for (let i = 0; i < count; i++) {
			...
			decor(
				ctx,
				Math.min(doorX, doorX + facing * 0.03),
				1.0,
				handleZ - 0.025,
				Math.max(doorX, doorX + facing * 0.03),
				1.16,
				handleZ + 0.025,
				PALETTE.metal,
			)
...
/** Скамейка. */
export function bench(ctx: Ctx, axis: "x" | "z", at: number, from: number, to: number, offset: number): void {
	if (axis === "x") {
...
/** Радиатор под окном. */
export function radiator(ctx: Ctx, axis: "x" | "z", at: number, from: number, to: number, facing: 1 | -1): void {
	const depth = 0.12
...
export function noticeBoard(
	...
	for (let i = 0; i < papers; i++) {
		...
		const y = 1.25 + n * 0.55
		const tone: Color = n > 0.72 ? PALETTE.plasticYellow : n > 0.45 ? PALETTE.paper : shade(PALETTE.glass, 1.1)
		if (side === "x0" || side === "x1") {
...
/** Стенные часы. */
export function wallClock(ctx: Ctx, side: "x0" | "x1" | "z0" | "z1", at: number, along: number, y = 2.5): void {
	const r = 0.24
	const d = 0.07
	if (side === "x0" || side === "x1") {
...
export function canteenTable(ctx: Ctx, x: number, z: number, length = 2.6): void {
...
/** Раздаточная линия. */
export function servingCounter(ctx: Ctx, x0: number, z0: number, x1: number, z1: number): void {
...
/** Торговый автомат. */
export function vendingMachine(ctx: Ctx, x: number, z: number, facing: "x0" | "x1" | "z0" | "z1"): void {
	const w = 0.9
	const d = 0.7
	const h = 1.9
...
export function readingTable(ctx: Ctx, x: number, z: number, radius = 0.85): void {
...
export function basketballHoop(ctx: Ctx, x: number, z: number, facing: 1 | -1): void {
	const boardY = 2.75
	...
	for (let i = 0; i < 4; i++) {
		...
		decor(
			...
			x - 0.22 + inset,
			y,
			Math.min(armZ, rimZ) + inset,
			x + 0.22 - inset,
			y + 0.02,
			Math.max(armZ, rimZ) - inset,
			PALETTE.paper,
		)
	}
}
...
export function bleachers(ctx: Ctx, x0: number, z0: number, z1: number, steps = 4): void {
	const stepDepth = 0.7
...
export function receptionDesk(ctx: Ctx, x0: number, z0: number, x1: number, z1: number): void {
...
/** Витрина с кубками. */
export function trophyCase(ctx: Ctx, side: "x0" | "x1", at: number, from: number, to: number, seed = 1): void {
	const depth = 0.45
	const x0 = side === "x0" ? at : at - depth
	const x1 = x0 + depth
	...
	for (let s = 0; s < 3; s++) {
		...
		for (let i = 0; i < items; i++) {
			...
			const h = 0.16 + n * 0.16
			const gold: Color = n > 0.6 ? [0.86, 0.72, 0.3] : [0.72, 0.74, 0.78]
...
export function schoolCrest(ctx: Ctx, x: number, y: number, z: number, size = 1.6): void {
	const d = 0.06
...
export function banner(ctx: Ctx, x: number, z: number, top: number, height: number, color: Color): void {
...
export function sinkRow(ctx: Ctx, x0: number, x1: number, z: number, facing: 1 | -1): void {
	const depth = 0.5
...
export function toiletStalls(ctx: Ctx, x: number, z0: number, z1: number, width: number, count: number): void {
...
export function crates(ctx: Ctx, x0: number, z0: number, x1: number, z1: number, seed = 1): void {
	let i = 0
	for (let x = x0; x < x1 - 0.5; x += 0.75) {
		for (let z = z0; z < z1 - 0.5; z += 0.8) {
			...
			if (n > 0.7) solid(ctx, x + 0.05, h, z + 0.05, x + 0.5, h + 0.35, z + 0.5, shade(tone, 0.9))
		}
	}
}

/** Дверное полотно, открытое вдоль стены (проход свободен). */
export function openDoorLeaf(
	ctx: Ctx,
	axis: "x" | "z",
	at: number,
	edge: number,
 ...
 */
export function hideLocker(
	ctx: Ctx,
	at: number,
	x: number,
	facing: 1 | -1,
	base: Color = PALETTE.lockerTeal,
): void {
	const depth = 0.78
	const height = 2.02
	const half = 0.46
	...
	const leaf = (y0: number, y1: number): void => {
