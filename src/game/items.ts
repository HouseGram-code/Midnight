...
import type { MeshBuilder, Color } from "../core/mesh.js"
import { PALETTE } from "../world/palette.js"

export type ItemKind = "key" | "crowbar" | "cutters" | "handle" | "fuse" | "flashlight"

export interface ItemDef {
	id: ItemKind
	/** Короткое название для HUD. */
	label: string
	/** Подсказка — где искать. */
	hint: string
	/** Зачем нужен на выходе. */
	use: string
	x: number
	y: number
	z: number
...
export const QUEST_ITEMS: readonly ItemDef[] = [
	{
		id: "key",
		label: "Ключ",
...
export const FLASHLIGHT_ITEM: ItemDef = {
	id: "flashlight",
	label: "Фонарь",
...
/** Шкафчики, в которых можно переждать погоню. */
export interface HideSpot {
	id: string
	/** У какой стены коридора стоит шкаф. */
	wall: "north" | "south"
	/** Куда встаёт игрок, чтобы открыть шкафчик. */
	x: number
	z: number
	/** Позиция камеры внутри — центр пустоты корпуса. */
	insideX: number
	insideZ: number
	/** Куда смотреть из шкафчика. */
	yaw: number
}
...
const GOLD: Color = [0.82, 0.68, 0.24]
const STEEL: Color = [0.68, 0.7, 0.74]
const STEEL_DARK: Color = [0.36, 0.38, 0.42]
const RUBBER: Color = [0.72, 0.22, 0.2]
const CERAMIC: Color = [0.9, 0.88, 0.82]
const COPPER: Color = [0.78, 0.5, 0.26]
const PLASTIC: Color = [0.16, 0.17, 0.19]
const LENS: Color = [1, 0.94, 0.72]
const BOARD: Color = [0.55, 0.4, 0.25]
const BOARD_DARK: Color = [0.44, 0.31, 0.19]
 ...
 */
export function buildItemPickup(
	mesh: MeshBuilder,
	def: ItemDef,
	time: number,
	highlighted: boolean,
	...
	const glow: Color = highlighted ? [1, 0.86, 0.4] : [0.5, 0.62, 0.8]
 ...
 */
export function buildBarricade(mesh: MeshBuilder, progress: number, time: number): void {
	const x0 = EXIT_DOOR.x - EXIT_DOOR.width / 2
	const x1 = EXIT_DOOR.x + EXIT_DOOR.width / 2
	const z = 43.6
	const planks: Array<{ y: number; angle: number; removeAt: number }> = [
	...
	for (const plank of planks) {
		if (progress > plank.removeAt) continue
	...
	mesh.box(
		...
		1.48,
		z - 0.06,
		locked ? ([1, 0.2, 0.16] as Color) : ([0.3, 1, 0.4] as Color),
		{ emissive: true },
	)
 ...
 */
export function buildExitDoors(mesh: MeshBuilder, angle: number): void {
	const halfWidth = EXIT_DOOR.width / 2
	const leafWidth = halfWidth - 0.05
	const z = 43.95
	const cos = Math.cos(angle)
	const sin = Math.sin(angle)
	const leaf = (hingeX: number, direction: 1 | -1): void => {
...
export function buildClassDoor(mesh: MeshBuilder, angle: number): void {
 ...
 */
const CHALK_STROKES: ReadonlyArray<{ z0: number; z1: number; y: number }> = [
...
const CHALK_COLOR: Color = [0.9, 0.92, 0.88]

/** count — сколько штрихов уже написано (дробное значение округляется вниз). */
export function buildChalkMarks(mesh: MeshBuilder, count: number): void {
