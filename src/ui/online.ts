...
export class OnlinePanel {
	...
	private renderMatchRoster(info: MatchInfo): void { this.rosterEl.replaceChildren(); for (const player of info.players) this.rosterEl.append(this.rosterRow(player.name, player.index, player.skin, player.id === this.session?.id, player.owner, player.id === info.host)) }
	private rosterRow(name: string, index: number, skin: number, self: boolean, owner: boolean, host: boolean): HTMLElement { const row = document.createElement("div"); row.className = `${self ? "online-slot online-slot--me" : "online-slot"}${owner ? " online-slot--owner" : ""}${host ? " online-slot--host" : ""}`; const dot = document.createElement("i"); dot.style.background = onlineSkinFor(skin, index).tag; const label = document.createElement("span"); label.textContent = self ? `${name} (вы)` : name; row.append(dot); if (host) { const hostMark = document.createElement("b"); hostMark.className = "online-slot__host"; hostMark.textContent = "✓"; hostMark.title = "Создатель комнаты"; hostMark.setAttribute("aria-label", "Создатель комнаты"); row.append(hostMark) } if (owner) { const mark = document.createElement("b"); mark.className = "creator-emblem creator-emblem--small"; mark.title = "Официальный создатель игры"; row.append(mark) } row.append(label); return row }
