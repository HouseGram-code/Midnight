...
import type { SpawnPoint } from "../world/layout.js"
...
export class PlayerController {
	...
	crouching = false
	sprinting = false
	height: number = CONFIG.player.height
	...
	teleport(spawn: SpawnPoint): void {
		this.x = spawn.x
		this.z = spawn.z
		this.y = 0
		this.yaw = spawn.yaw
	...
	private moveHorizontal(dx: number, dz: number): void {
		...
		if (this.onGround) {
			const stepY = this.collision.groundHeight(
				nextX,
				nextZ,
				this.y,
				radius,
		...
		if (dx !== 0) this.velocityX = 0
		if (dz !== 0) this.velocityZ = 0
	}
