...
import type { NetPayload } from "./realtime.js"
...
export interface LocalSnapshot {
	...
	escaped: boolean
	down: boolean
	/** Мы реально бегаем по школе: не кат-сцена, не смерть, не победа, не пауза. */
	active: boolean
}
...
export interface NetPlayer {
	...
	escaped: boolean
	down: boolean
	/** Игрок в игре и может вести учительницу. */
	active: boolean
	phase: number
	last: number
}
...
export interface OnlineHooks {
	...
	onItem: (kind: string, who: string) => void
	/** Кто-то продвинул баррикаду. */
	onStage: (stage: number, who: string) => void
	/** Хост сообщил, что ударили нас. */
	onHit: () => void
	/** Свет выключился (решает хост). */
	onLightsOut: () => void
...
export class OnlineGame {
	...
	teacherView: TeacherSnapshot | null = null
	live = true
	...
	constructor(
		...
		private readonly hooks: OnlineHooks,
	) {
		...
		for (const player of match.players) {
			...
			if (player.id === session.id) continue
	...
	get playerCount(): number {
		let alive = 1
		for (const player of this.players.values()) if (!player.down) alive += 1
		return alive
	}
	...
	get authority(): boolean {
		if (!this.live || !this.localActive) return false
		for (const player of this.players.values()) {
			if (!player.active) continue
			if (player.index < this.localIndex) return false
		}
		return true
	}

	/** true ровно в тот кадр, когда мы взяли учительницу на себя. */
	takeAuthority(): boolean {
	...
	private bind(): void {
		...
		room.on("i", (payload) => {
			...
			if (kind) this.hooks.onItem(kind, this.nameOf(id))
		})
		room.on("g", (payload) => {
			const id = String(payload.i ?? "")
			if (id === this.session.id) return
		...
		room.on("h", (payload) => {
			if (String(payload.i ?? "") !== this.session.id) return
			this.hooks.onHit()
		})
		...
		room.on("e", (payload) => {
			...
			if (player) player.escaped = true
			if (id !== this.session.id) this.hooks.onToast(`${this.nameOf(id)} выбрался из школы!`)
		})
		room.on("d", (payload) => {
			const id = String(payload.i ?? "")
			const player = this.players.get(id)
			if (player) player.down = true
			if (id !== this.session.id) this.hooks.onToast(`${this.nameOf(id)} попался…`)
		})
		room.on("bye", (payload) => {
			const id = String(payload.i ?? "")
	...
	private applyState(payload: NetPayload): void {
		const id = String(payload.i ?? "")
		if (!id || id === this.session.id) return
	...
	private applyTeacherPacket(payload: NetPayload): void {
		// Свою учительницу чужими пакетами не перебиваем.
		if (this.authority) return
		const next: TeacherSnapshot = {
		...
		if (!this.teacherView) this.teacherView = { ...next }
		this.teacherStale = 0
	}

	/** Отправка своего состояния и сглаживание чужих. */
	update(dt: number, local: LocalSnapshot): void {
		...
		for (const player of this.players.values()) {
			...
			if (player.last > 0 && now - player.last > 1000) player.speed = 0
		}
	}

	/** Хост рассылает учительницу. */
	pushTeacher(dt: number, snapshot: TeacherSnapshot): void {
		if (!this.authority) return
	 ...
	 */
	private updateTeacherView(dt: number): void {
	...
	buildMesh(mesh: MeshBuilder): void {
		for (const player of this.players.values()) {
			// В шкафчике и после побега не рисуем вовсе.
			if (player.hidden || player.escaped || player.down) continue
