...
function buildShirtPrint(mesh: MeshBuilder, hipY: number, chestTop: number): void {
...
export interface RemotePose {
	...
	z: number
	yaw: number
	/** Скорость для анимации шага. */
	speed: number
	phase: number
	crouching: boolean
	/** Сидит за партой — вступительная кат-сцена. */
	sitting: boolean
	/** С фонарём в руке. */
	flashlight: boolean
	/** Предмет в руке: рисуем настоящую модель, а не палку. */
	held?: ItemKind | null
	skin: PlayerSkin
}
 ...
 */
export function buildRemotePlayer(mesh: MeshBuilder, pose: RemotePose): void {
	...
	if (skin.id === "ryzik3489") buildShirtPrint(mesh, hipY, chestTop)
	...
	const held: ItemKind | null = sit ? null : (pose.held ?? (pose.flashlight ? "flashlight" : null))
	if (held) {
		...
		buildItemModel(
			mesh,
			held,
			{ x: 0.255, y: handY, z: -0.5, yaw: 0 },
			0.95 * scale,
			held === "flashlight" && pose.flashlight,
		)
...
export function projectToScreen(
	...
	if (forward <= 0.15) return { x: 0, y: 0, visible: false, distance }
