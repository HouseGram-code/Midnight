...
async function boot() {
    ...
    for (const chunk of scene.chunks)
        renderer.uploadChunk(chunk.name, chunk.mesh);
    renderer.resize();
    ...
    for (const control of Array.from(document.querySelectorAll(".setting__range, .setting__check"))) {
    ...
    const onlinePanel = new OnlinePanel({
        ...
        onMatch: (session, match) => {
            ...
            netHud.reset();
            netHud.setSelf(session.name, onlineSkinFor(me?.skin ?? 0, me?.index ?? 0).tag, session.owner, match.host === session.id, me?.skin === 1 ? "ryzik3489" : "classic");
    ...
    input.onKey("KeyT", () => {
        if (!game.online || menu.isVisible || game.paused)
            return;
        netHud.toggleChat();
    });
    ...
    const grabMouse = () => {
        if (menu.isVisible || !wantsPointerLock)
            return;
    ...
    document.addEventListener("pointerlockchange", () => {
        ...
        if (game.state === "intro" || game.state === "outro" || game.state === "hurt")
            return;
        pauseGame();
    });
...
void boot().catch((error) => overlay.showError(describe(error)));
