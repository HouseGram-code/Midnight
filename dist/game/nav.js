...
export class NavGraph {
    ...
    nearestFree(x, z, test, maxDistance = 14) {
        let best;
    ...
    roamTarget(fromX, fromZ, minDistance = 12, test) {
        if (this.roamIds.length === 0)
            return undefined;
        let fallback;
        for (let attempt = 0; attempt < 24; attempt++) {
            ...
            if (!node)
                continue;
            if (test && !test(node.x, node.z))
                continue;
            fallback = node;
            if (Math.hypot(node.x - fromX, node.z - fromZ) >= minDistance)
                return node;
        }
        return fallback;
    }
    ...
    path(fromX, fromZ, toX, toZ) {
        ...
        if (!start || !goal)
            return [{ x: toX, z: toZ }];
        if (start.id === goal.id)
            return [{ x: goal.x, z: goal.z }, { x: toX, z: toZ }];
