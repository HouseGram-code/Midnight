...
export class Game {
    ...
    ghostY = CONFIG.player.eyeHeight;
    ghostTimer = 0;
    holdProgress = 0;
    holdActive = false;
    holdKey = false;
    ...
    startOnlineGame(session, match, ui) {
    ...
    netTags(width, height) {
        ...
        if (this.paused)
            return [];
        if (this.state === "outro" || this.state === "dead" || this.state === "won")
            return [];
        const tags = [];
    ...
    tagInSight(id, head) {
        ...
        if (now - probe.at < 120)
            return probe.clear;
        probe.at = now;
        probe.clear = !this.collision.blocked(this.camera.x, this.camera.y, this.camera.z, head.x, head.y, head.z);
        return probe.clear;
    }
    refreshItemsHud() {
        this.hud.setItems(QUEST_ITEMS.map((item) => ({ label: item.label, found: this.collected.has(item.id) })));
    }
    say(text) {
     ...
     */
    sceneCamera(x, y, z, yaw, pitch, fov) {
    ...
    seatSway(lookAt, weight = 1) {
        ...
        this.setCamera(SEAT.x + Math.sin(this.time * 0.6) * 0.012, SEAT.y + Math.sin(this.time * 1.3) * 0.006, SEAT.z + Math.cos(this.time * 0.5) * 0.012, yaw + sway, -0.06 + Math.sin(this.time * 0.7) * 0.012, 70);
    }
    /**
        ...
        const steps = [
            ...
            },
            {
                ...
                onUpdate: (progress, dt) => {
                    ...
                    if (this.stepTimer <= 0) {
                        this.stepTimer = 0.46;
                        this.audio.playAt("teacher_step", { x: this.teacher.x, z: this.teacher.z }, { x: this.camera.x, z: this.camera.z, yaw: this.camera.yaw }, { volume: 0.75, range: 20 });
                    }
            ...
            },
            {
                ...
                onUpdate: (progress) => {
                    ...
                    this.sceneCamera(mix(SEAT.x, STAND_SPOT.x, t), mix(SEAT.y, CONFIG.player.eyeHeight, t), mix(SEAT.z, STAND_SPOT.z, t), mix(SEAT.yaw, STAND_SPOT.yaw, t), mix(-0.06, 0, t));
                },
            },
        ];
        ...
        const steps = [
            {
                ...
                onUpdate: (progress) => {
                    ...
                    this.sceneCamera(mix(startX, EXIT_DOOR.x, t), mix(CONFIG.player.eyeHeight, 1.45, t), mix(startZ, EXIT_DOOR.standZ - 0.4, t), mixAngle(this.player.yaw, 0, t), mix(this.player.pitch, -0.22, t), mix(74, 58, t));
                },
            },
            {
                id: "open",
                ...
                onUpdate: (progress) => {
                    ...
                    this.sceneCamera(EXIT_DOOR.x, mix(1.45, CONFIG.player.eyeHeight, t), mix(EXIT_DOOR.standZ - 0.4, EXIT_DOOR.standZ + 0.6, t), 0, mix(-0.22, 0.02, t), mix(58, 74, t));
                },
            },
            {
                id: "step",
                ...
                onUpdate: (progress) => {
                    const t = easeInOut(progress);
                    this.sceneCamera(EXIT_DOOR.x + Math.sin(progress * 4) * 0.05, CONFIG.player.eyeHeight + Math.sin(progress * 12) * 0.02, mix(EXIT_DOOR.standZ + 0.6, 45.8, t), Math.sin(progress * 3) * 0.03, 0.01, mix(74, 78, t));
                },
            },
            {
                id: "run",
                ...
                onUpdate: (progress) => {
                    const t = easeInOut(progress);
                    this.sceneCamera(EXIT_DOOR.x + Math.sin(progress * 9) * 0.13, CONFIG.player.eyeHeight + Math.sin(progress * 34) * 0.035, mix(45.8, 52.5, t), Math.sin(progress * 5) * 0.05, 0.02, mix(78, 86, t));
                },
                onExit: () => {
            ...
            },
            {
                ...
                onUpdate: (progress) => {
                    ...
                    this.sceneCamera(EXIT_DOOR.x, CONFIG.player.eyeHeight, 52.5, mix(0, Math.PI, t), 0.02, mix(86, 70, t));
            ...
            },
            {
                ...
                onUpdate: (progress) => {
                    const t = easeInOut(progress);
                    this.sceneCamera(EXIT_DOOR.x + mix(0, 2.6, t), CONFIG.player.eyeHeight + mix(0, 0.32, t), mix(52.5, 57.5, t), mix(Math.PI, Math.PI * 0.82, t), mix(0.02, 0.06, t), 70);
        ...
        const steps = [
            {
                ...
                onUpdate: (progress) => {
                    ...
                    const t = easeOut(progress);
                    this.fade = t * 0.28;
                    this.setCamera(this.player.x, mix(this.player.eyeY, 0.42, t), this.player.z, this.player.yaw + t * 0.55, mix(fromPitch, -0.85, t));
                },
            },
            {
                id: "blackout",
                ...
                onUpdate: (progress) => {
                    ...
                    this.setCamera(this.player.x, mix(0.42, 0.32, t), this.player.z, this.player.yaw + 0.55, -0.85);
                },
                onExit: () => {
                    this.fade = 1;
            ...
            },
            {
                ...
                onUpdate: (progress) => {
                    ...
                    this.setCamera(spot.x, mix(0.3, CONFIG.player.eyeHeight, t), spot.z, spot.yaw, mix(-0.8, 0, t));
                },
                onExit: () => {
                    this.say(null);
                },
    ...
    toggleFlashlight() {
        ...
        if (this.flashlightOn && this.heldTimer <= 0)
            this.heldItem = "flashlight";
    }
    // ------------------------------------------------------------ пояс предметов
    ...
    selectSlot(index) {
        ...
        if (changed)
            this.audio.play("click", { volume: 0.55, rate: 1.15 });
    ...
    refreshHotbarHud() {
        ...
        this.hud.setHotbar(this.hotbar.map((kind, index) => ({
            key: String(index + 1),
    ...
    findTarget() {
        ...
        const consider = (x, y, z, make) => {
            const dx = x - eyeX;
            const dz = z - eyeZ;
            const distance = Math.hypot(dx, dz);
            if (distance > CONFIG.horror.reach)
                return;
            if (Math.abs(y - eyeY) > 1.8)
                return;
    ...
    completeInteraction(target) {
        if (target.kind === "item" && target.item) {
            ...
            this.refreshItemsHud();
            this.refreshHotbarHud();
            this.hud.toast(`${item.label}: найдено ${this.collected.size} из 5 · клавиша ${slot + 1} — взять в руку`, 3.8);
    ...
    triggerLightsOut() {
        ...
        if (node)
            ...
            this.teacher.awaken(69, 15, Math.PI);
    }
    // -------------------------------------------------------------------- кадр
    /**
     * Один кадр игры.
     ...
     */
    frame(frameSeconds, steps, fixedStep, mouse, invertY) {
    ...
    updateHiding(dt, mouse, invertY) {
        ...
        const delta = Math.atan2(Math.sin(this.player.yaw - this.hideYaw), Math.cos(this.player.yaw - this.hideYaw));
        ...
        this.setCamera(spot.insideX, 1.42 + Math.sin(this.time * 1.7) * 0.005, spot.insideZ, this.player.yaw, this.player.pitch, 60);
        this.target = null;
        this.hud.setHold(null);
        this.hud.setPrompt("E — выйти из шкафчика");
        this.hud.setObjective(this.hideSpotted
    ...
    updateFootsteps(dt) {
        ...
        if (this.stepsOn) {
            ...
            this.audio.setLoopVolume("steps", CONFIG.audio.steps * (crouching ? 0.4 : sprinting ? 1.15 : 1));
        }
    ...
    updateAtmosphere(dt) {
        ...
        if (this.whisperTimer <= 0) {
            ...
            if (this.lightsOut) {
                ...
                this.audio.playAt("whisper", { x: this.player.x + Math.cos(angle) * 6, z: this.player.z + Math.sin(angle) * 6 }, this.player, { volume: 0.45, range: 15 });
            }
        }
    ...
    updateTeacher(dt) {
        ...
        if (online && online.takeAuthority()) {
            ...
            if (net) {
                if (this.lightsOut)
                    this.teacher.awaken(net.x, net.z, net.yaw);
                else
                    this.teacher.placeAt(net.x, net.z, net.yaw);
                this.teacher.visible = net.visible;
            }
        ...
        if (online) {
            ...
            for (const other of online.players.values()) {
                if (other.hidden || other.escaped || other.down)
                    continue;
        ...
        this.teacher.update(dt, {
            ...
            onSwing: () => {
                this.audio.playAt("swing", { x: this.teacher.x, z: this.teacher.z }, this.player, { volume: 0.8, range: 12 });
            },
            onImpact: () => {
    ...
    updateDynamicMeshes() {
        ...
        const sway = Math.sin(this.time * 1.7) * 0.006 + (walking ? Math.sin(this.time * 9.4) * 0.018 : 0);
    ...
    get environment() {
        ...
        return {
            ...
            flash: this.flashlightOn && this.flashlightOwned
                ? {
                    x: this.camera.x,
                    y: this.camera.y - 0.1,
                    z: this.camera.z,
