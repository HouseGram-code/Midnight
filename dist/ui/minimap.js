...
export class Minimap {
    ...
    constructor(canvas) {
        ...
        if (!context)
            throw new Error("Не удалось создать 2D контекст миникарты");
        this.context = context;
        const padding = 10;
        this.scale = Math.min((canvas.width - padding * 2) / BUILDING.width, (canvas.height - padding * 2) / BUILDING.depth);
    ...
    drawPlan() {
        ...
        for (const room of ROOMS) {
            ...
            ctx.fillRect(this.mapX(room.x0), this.mapY(room.z0), (room.x1 - room.x0) * this.scale, (room.z1 - room.z0) * this.scale);
        }
        // стены
        ...
        for (const wall of WALLS) {
            for (const door of wall.doors ?? []) {
                ...
                const color = door.kind === "glass" ? "#2783de" : door.kind === "arch" ? "#9a958c" : "#d2691e";
        ...
        for (const room of ROOMS) {
            ...
            ctx.fillText(label, this.mapX((room.x0 + room.x1) / 2), this.mapY((room.z0 + room.z1) / 2), width - 8);
        }
        ...
        ctx.lineWidth = 1;
        ctx.strokeRect(this.mapX(0) - 0.5, this.mapY(0) - 0.5, BUILDING.width * this.scale + 1, BUILDING.depth * this.scale + 1);
    }
    render(x, z, yaw, now) {
        if (this.canvas.hidden || now < this.nextUpdate)
            return;
