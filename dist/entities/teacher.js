 ...
 */
import { MeshBuilder, shade } from "../core/mesh.js";
...
const SKIN = [0.86, 0.76, 0.7];
const SKIN_DARK = [0.68, 0.58, 0.55];
const DRESS = [0.17, 0.15, 0.2];
const DRESS_LIGHT = [0.26, 0.23, 0.3];
const BLOUSE = [0.79, 0.78, 0.8];
const HAIR = [0.11, 0.1, 0.12];
const SHOE = [0.09, 0.08, 0.09];
const EYE = [1, 0.12, 0.1];
const CLUB = [0.42, 0.28, 0.16];
const CLUB_TAPE = [0.13, 0.13, 0.14];
/** Маска, которая появляется после кат-сцены: чёрное лицо и светящиеся черты. */
const MASK = [0.02, 0.02, 0.025];
const MASK_GLOW = [0.97, 0.97, 0.93];
export class Teacher {
    ...
    setAnimationSpeed(value, dt = 0) {
        this.speed = value;
        if (value > 0.1)
            this.walkPhase += dt * value * 3.2;
    }
    stun(seconds) {
    ...
    retreatFrom(x, z, minDistance = 26) {
        const node = this.nav.roamTarget(x, z, minDistance);
        if (node)
            this.placeAt(node.x, node.z, this.yaw);
        this.state = "patrol";
        this.alert = 0;
        this.memory = 0;
        this.seesPlayer = false;
    ...
    hasLineOfSight(toX, toZ, eyeY) {
        ...
        for (let i = 1; i < steps; i++) {
            ...
            if (this.collision.overlaps(px, y - 0.25, pz, 0.05, 0.5))
                return false;
        }
        return true;
    }
    ...
    setTarget(x, z, force = false) {
        ...
        if (!force && moved < 2 && this.path.length > 0)
            return;
        this.targetX = x;
        this.targetZ = z;
    ...
    findPath(goalX, goalZ) {
        ...
        while (head < tail && !reached) {
            ...
            const ci = current % w;
            const cj = (current - ci) / w;
            for (let k = 0; k < 8; k++) {
                const ni = ci + NEIGHBOURS[k * 2];
                const nj = cj + NEIGHBOURS[k * 2 + 1];
                if (ni < 0 || nj < 0 || ni >= w || nj >= h)
                    continue;
                const next = nj * w + ni;
                if (from[next] !== -1 || grid[next] === 0)
                    continue;
                // Диагональ разрешена только когда свободны обе соседние клетки.
                if (ni !== ci && nj !== cj) {
                    if (grid[cj * w + ni] === 0 || grid[nj * w + ci] === 0)
                        continue;
                }
                from[next] = current;
                queue[tail++] = next;
    ...
    walkClear(fromX, fromZ, toX, toZ) {
        const dx = toX - fromX;
        const dz = toZ - fromZ;
        const distance = Math.hypot(dx, dz);
        if (distance < 0.001)
            return !this.blocked(toX, toZ);
        const steps = Math.min(64, Math.max(2, Math.ceil(distance / 0.28)));
        for (let i = 1; i <= steps; i++) {
            const t = i / steps;
            if (this.blocked(fromX + dx * t, fromZ + dz * t))
                return false;
        }
        return true;
    }
    tryMove(dx, dz) {
        ...
        for (let i = 0; i < steps; i++) {
            ...
            if (this.blocked(nx, nz, TEACHER.moveRadius)) {
                if (this.blocked(nx, nz, TEACHER.navRadius))
                    return moved;
            }
            this.x = nx;
            this.z = nz;
    ...
    unstuck() {
        ...
        for (const radius of [0.3, 0.55, 0.85]) {
            ...
            for (let i = 0; i < 16; i++) {
                ...
                if (!embedded && !this.walkClear(this.x, this.z, nx, nz))
                    continue;
                this.x = nx;
                this.z = nz;
                return;
    ...
    advance(dt, speed, events) {
        ...
        for (let guard = 0; guard < 8 && this.path.length > 1; guard++) {
            ...
            if (!this.walkClear(this.x, this.z, next.x, next.z))
                break;
            this.path.shift();
        }
        ...
        if (!moved) {
            const other = this.slideSign === 1 ? -1 : 1;
    ...
    update(dt, senses, events = {}) {
        ...
        this.lookTimer += dt;
        if (this.screamTimer > 0)
            this.screamTimer = Math.max(0, this.screamTimer - dt);
        ...
        if (inSight)
            ...
            this.sightTime = Math.max(0, this.sightTime - dt * 2);
        const locked = this.state === "chase" || this.state === "attack";
        const sees = inSight && (locked || this.sightTime >= TEACHER.noticeTime * this.tuning.notice);
        ...
        if (sees || hears) {
            ...
            if (senses.playerHidden)
                this.memory = Math.min(this.memory, 2);
        ...
        switch (this.state) {
            case "attack": {
                ...
                if (this.attackTimer < TEACHER.attackDuration * 0.62) {
                    ...
                    if (gap > 0.85) {
                        this.tryMove(-Math.sin(this.yaw) * TEACHER.lungeSpeed * dt, -Math.cos(this.yaw) * TEACHER.lungeSpeed * dt);
                ...
                this.attackClosest = Math.min(this.attackClosest, this.distanceTo(senses.playerX, senses.playerZ));
                const reachable = this.attackClosest <= TEACHER.attackReach &&
            ...
            case "chase": {
                if (distance <= TEACHER.attackRange &&
                    this.cooldown <= 0 &&
                    (!senses.playerHidden || senses.playerHiddenSpotted)) {
                    this.state = "attack";
                    this.attackTimer = 0;
                ...
                const chaseSpeed = this.catchTimer > 0
                    ? TEACHER.catchSpeed
                    : rush
                        ? TEACHER.sprintSpeed
                        : TEACHER.chaseSpeed;
                this.advance(dt, chaseSpeed * this.tuning.speed, events);
                break;
            }
            case "search": {
                this.searchTimer -= dt;
                ...
                if (this.path.length === 0 || this.pathTimer > 3) {
                    ...
                    for (let attempt = 0; attempt < 8 && !picked; attempt++) {
                        ...
                        if (this.blocked(tx, tz))
                            continue;
                        this.setTarget(tx, tz, true);
                        picked = true;
                    }
                    if (!picked)
                        this.setTarget(this.lastSeenX, this.lastSeenZ, true);
                    if (this.path.length === 0) {
                        this.startLook(0.4);
                        break;
                    }
                }
                this.advance(dt, TEACHER.searchSpeed * this.tuning.speed, events);
                break;
            }
            case "patrol":
            default: {
                ...
                if (this.path.length === 0 || this.pathTimer > 8) {
                    ...
                    if (node)
                        this.setTarget(node.x, node.z, true);
                    if (this.path.length === 0) {
                        this.startLook(0.4);
                        break;
                    }
                }
                this.advance(dt, TEACHER.patrolSpeed * this.tuning.speed, events);
    ...
    part(mesh, lx, ly, lz, sx, sy, sz, color, emissive = false) {
        ...
        mesh.rotatedBox(wx, this.y + oy, wz, sx, sy, sz, this.yaw, color, emissive ? { emissive: true } : undefined);
    }
    /**
     ...
     */
    faceMask(mesh, bob, headTurn, scream = 0) {
        ...
        const box = (cx, cy, cz, sx, sy, sz, color, glow = false) => {
        ...
        for (let i = 0; i < teeth; i += 1) {
            ...
            box(t * 0.19, base + 0.05 + lift - open * 0.4, faceZ, 0.017, tall, 0.013, MASK_GLOW, true);
        }
    ...
    buildMesh(mesh, options = {}) {
        ...
        const bob = (moving ? Math.abs(Math.cos(this.walkPhase)) * 0.035 : 0) + screamPow * 0.05;
        ...
        // Голова, волосы, пучок.
        const headTurn = (writing ? 0 : Math.sin(this.lookTimer * 0.7) * 0.02) + screamPow * 0.055;
