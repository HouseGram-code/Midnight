...
export function findFloor2RoomAt(x, z) {
...
// ─────────────────── примитивы ───────────────────
function solid(ctx, x0, y0, z0, x1, y1, z1, color) {
...
function decor(ctx, x0, y0, z0, x1, y1, z1, color) {
    ctx.mesh.box(x0, y0, z0, x1, y1, z1, color);
}
function glow(ctx, x0, y0, z0, x1, y1, z1, color) {
    ctx.mesh.box(x0, y0, z0, x1, y1, z1, color, { emissive: true });
}
function door(at, width = 1.1, height = 2.15) {
    return { at, width, y0: BASE, y1: BASE + height };
}
function window2(at, width = 2.2, sill = 0.95, height = 1.75) {
    return { at, width, y0: BASE + sill, y1: BASE + sill + height, glass: true };
}
/** Стена второго этажа с проёмами. */
function wallRun(ctx, axis, at, from, to, thickness, color, openings = [], top = TOP) {
    const half = thickness / 2;
    const put = (u0, u1, y0, y1, c, blocking = true) => {
        if (u1 - u0 < 1e-4 || y1 - y0 < 1e-4)
            return;
        if (axis === "x") {
            if (blocking)
                solid(ctx, u0, y0, at - half, u1, y1, at + half, c);
            else
                decor(ctx, u0, y0, at - half, u1, y1, at + half, c);
        }
        else {
            if (blocking)
                solid(ctx, at - half, y0, u0, at + half, y1, u1, c);
            else
                decor(ctx, at - half, y0, u0, at + half, y1, u1, c);
        }
    };
    ...
    for (const opening of sorted) {
        ...
        if (opening.y0 > BASE)
            put(a, b, BASE, opening.y0, color);
        if (opening.y1 < top)
            put(a, b, opening.y1, top, color);
        if (opening.glass)
            put(a, b, opening.y0, opening.y1, PALETTE.glass, false);
        cursor = Math.max(cursor, b);
    }
    put(cursor, to, BASE, top, color);
}
/** Плита пола второго этажа + потолок над помещением. */
function slab(ctx, x0, z0, x1, z1, floorColor, tile = 1.4) {
...
function lamps(ctx, lights, x0, z0, x1, z1, columns, rows, long = 1.3) {
    const y = TOP - 0.16;
    for (let r = 0; r < rows; r++) {
...
function chair2(ctx, x, z, color = PALETTE.plasticBlue) {
...
function rackRow(ctx, x0, z, x1, seed) {
    const height = 2.25;
...
function serverRack(ctx, x, z, seed) {
    ...
    for (let i = 0; i < 9; i++) {
        ...
        const color = n > 0.62 ? [0.2, 1, 0.4] : n > 0.3 ? [1, 0.72, 0.2] : [0.25, 0.6, 1];
        glow(ctx, x + 0.24, y + 0.04, z + 0.38, x + 0.33, y + 0.1, z + 0.39, color);
    }
}
/** Диван в учительской. */
function sofa(ctx, x0, z, x1) {
...
function railing(ctx, axis, at, from, to) {
    const y = BASE + 1.05;
    if (axis === "x") {
...
function buildServer(ctx, lights) {
    slab(ctx, 33, 0, 44, 12, PALETTE.floorTile, 1.1);
    lamps(ctx, lights, 33, 0, 44, 12, 2, 2);
    for (let i = 0; i < 5; i++)
        serverRack(ctx, 34.4 + i * 1.05, 1.2, i + 2);
    for (let i = 0; i < 4; i++)
        serverRack(ctx, 40.2 + i * 1.05, 3.6, i + 9);
...
function buildStaff(ctx, lights) {
...
function buildPrincipal(ctx, lights) {
...
function buildStore2(ctx, lights) {
...
function buildCorridor2(ctx) {
    for (let i = 0; i < 6; i++) {
        const x0 = i * 12;
        const ctx2 = { mesh: ctx.get(`floor2-corridor-${i}`), collision: ctx.collision };
...
function buildAssembly(ctx, lights) {
...
function buildShell(ctx) {
    const north = { mesh: ctx.get("floor2-shell-n"), collision: ctx.collision };
    const south = { mesh: ctx.get("floor2-shell-s"), collision: ctx.collision };
    const inner = { mesh: ctx.get("floor2-shell-i"), collision: ctx.collision };
    // северный фасад с окнами каждого кабинета
    const northWindows = [];
    for (const x0 of [11, 22, 33, 44, 55]) {
        for (const at of [x0 + 2.1, x0 + 5.5, x0 + 8.9])
            northWindows.push(window2(at));
    }
    ...
    const stair = { mesh: ctx.get("floor2-stairs"), collision: ctx.collision };
    ...
    const facade = { mesh: ctx.get("floor2-facade"), collision: ctx.collision };
    ...
    // крыша и парапет
    const roof = { mesh: ctx.get("floor2-roof"), collision: ctx.collision };
...
export function buildFloor2(ctx) {
    buildShell(ctx);
    buildCorridor2(ctx);
    const room = (name) => ({ mesh: ctx.get(name), collision: ctx.collision });
