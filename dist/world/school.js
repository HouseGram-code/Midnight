 ...
 */
import { MeshBuilder, hashNoise, shade } from "../core/mesh.js";
...
const LAMP = { radius: 8.5, intensity: 0.55, color: [1, 0.95, 0.86] };
const WINDOW_LIGHT = { radius: 13, intensity: 0.85, color: [0.82, 0.88, 1] };
const GRASS = [0.4, 0.47, 0.31];
const PAVEMENT = [0.58, 0.575, 0.55];
const TRUNK = [0.31, 0.24, 0.18];
const LEAVES = [0.25, 0.39, 0.25];
const DISTANT = [0.6, 0.61, 0.63];
class ChunkSet {
    map = new Map();
    get(name) {
...
function freeSpans(from, to, blockers, margin) {
...
// ──────────────────── СТЕНЫ ────────────────────
function chunkForWall(wall, along) {
    return `wall-${wall.axis}${wall.at}-${Math.floor(along / 18)}`;
}
function wallBox(mesh, wall, u0, u1, y0, y1, color, expand = 0, emissive = false) {
    if (u1 <= u0 || y1 <= y0)
        return;
...
function wallCollider(collision, wall, u0, u1, y0, y1) {
    if (u1 <= u0 || y1 <= y0)
        return;
...
function inwardSign(wall) {
    if (wall.axis === "x")
        return wall.at < BUILDING.depth / 2 ? 1 : -1;
    return wall.at < BUILDING.width / 2 ? 1 : -1;
}
function buildWindow(mesh, wall, win, lights) {
...
function buildSolidSpan(ctx, wall, u0, u1, windows, lights) {
    ...
    for (const win of windows) {
        ...
        const b = win.at + win.width / 2;
        if (a > cursor)
            wallBox(ctx.mesh, wall, cursor, a, 0, wall.height, wall.color);
        wallBox(ctx.mesh, wall, a, b, 0, win.sill, wall.color);
        const top = win.sill + win.height;
        if (top < wall.height)
            wallBox(ctx.mesh, wall, a, b, top, wall.height, wall.color);
        buildWindow(ctx.mesh, wall, win, lights);
        cursor = b;
    }
    if (cursor < u1)
        wallBox(ctx.mesh, wall, cursor, u1, 0, wall.height, wall.color);
    if (wall.wainscot) {
...
function doorSwing(wall, door) {
    ...
    const open = (room) => room !== null && room.kind !== "corridor" && room.kind !== "hall";
    if (open(plus))
        return 1;
    if (open(minus))
        return -1;
...
function buildDoorway(ctx, wall, door, lights) {
    ...
    // порог
    if (!isArch)
        wallBox(ctx.mesh, wall, a, b, 0, 0.025, PALETTE.metal, 0.05);
...
function buildWalls(chunks, collision, lights) {
    for (const wall of WALLS) {
        ...
        const spans = freeSpans(wall.from, wall.to, doors.map((door) => [door.at - door.width / 2, door.at + door.width / 2]), 0);
...
function buildRoomShell(room, ctx, lights) {
    const mesh = ctx.mesh;
    if (room.kind === "gym") {
...
function buildClassroom(room, ctx) {
    const x0 = room.x0;
    ...
    const posters = [PALETTE.plasticGreen, PALETTE.plasticYellow, PALETTE.plasticBlue];
    for (let i = 0; i < 3; i++) {
        const z = 2.4 + i * 2.3;
        P.decor(ctx, x0 + 10.85, 1.35, z, x0 + 10.87, 2.25, z + 0.7, posters[i]);
    }
}
function buildCorridor(room, chunks, collision, lights) {
    const slice = (x) => ({
    ...
    const fill = (side, at, facing, spans, seed) => {
        let index = 0;
    ...
    const hideRanges = (wall) => HIDE_SPOTS.filter((spot) => spot.wall === wall).map((spot) => [spot.x - 0.9, spot.x + 0.9]);
    fill("z0", 12.125, 1, freeSpans(0.45, 71.55, [...doorRanges(wallsAt("x", 12)), ...hideRanges("north")], 0.6), 3);
    fill("z1", 17.875, -1, freeSpans(0.45, 71.55, [...doorRanges(wallsAt("x", 18)), ...hideRanges("south")], 0.6), 8);
...
function buildHall(room, ctx) {
    ...
    const stepTopAt = (z) => Math.min(flight.topY, Math.max(0, ((z - 28.5) / 0.3) * 0.186));
    for (let i = 0; i < 5; i++) {
        const z = 28.6 + i * 1.55;
...
function buildCafeteria(room, ctx) {
...
function buildLibrary(room, ctx) {
...
function buildRestroom(room, ctx) {
...
function buildStorage(room, ctx) {
...
export function buildSchool() {
    ...
    for (const room of ROOMS) {
        ...
        const ctx = { mesh: chunks.get(`room-${room.id}`), collision };
    ...
    for (const [name, mesh] of chunks.map) {
        ...
        if (name === "outside") {
            ...
            bakeLighting(mesh, lightsForBounds(lights, mesh.minX, mesh.minY, mesh.minZ, mesh.maxX, mesh.maxY, mesh.maxZ));
        }
