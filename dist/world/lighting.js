...
export function bakeLighting(mesh, lights) {
...
export function bakeConstant(mesh, factor) {
    const vertices = mesh.emissive.length;
    for (let i = 0; i < vertices; i++) {
        if (mesh.emissive[i] > 0.5)
            continue;
...
export function lightsForBounds(lights, minX, minY, minZ, maxX, maxY, maxZ) {
    const result = [];
    for (const light of lights) {
        const r = light.radius;
        if (light.x + r < minX || light.x - r > maxX)
            continue;
        if (light.y + r < minY || light.y - r > maxY)
            continue;
        if (light.z + r < minZ || light.z - r > maxZ)
            continue;
        result.push(light);
    }
    return result;
