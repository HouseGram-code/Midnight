...
export class MeshBuilder {
    ...
    pushVertex(x, y, z, nx, ny, nz, color, emit) {
    ...
    quad(ax, ay, az, bx, by, bz, cx, cy, cz, dx, dy, dz, color, emissiveFlag = false) {
        const u1 = bx - ax;
        const u2 = by - ay;
        const u3 = bz - az;
        const v1 = cx - ax;
    ...
    /** Куб по двум углам. */
    box(x0, y0, z0, x1, y1, z1, color, options) {
    ...
    rotatedBox(centerX, bottomY, centerZ, sizeX, sizeY, sizeZ, angleY, color, options) {
        const hx = sizeX / 2;
        const hz = sizeZ / 2;
        ...
        const emit = options?.emissive ?? false;
        const skip = options?.skip ?? 0;
        const px = (lx, lz) => centerX + lx * c + lz * s;
        const pz = (lx, lz) => centerZ - lx * s + lz * c;
        const x00 = px(-hx, -hz);
        const z00 = pz(-hx, -hz);
        const x10 = px(hx, -hz);
        const z10 = pz(hx, -hz);
    ...
    horizontalQuad(x0, z0, x1, z1, y, color, facingUp = true, emissiveFlag = false) {
        if (facingUp) {
